...
class MORN(nn.Module):
    def __init__(self,
        ...
        self.cnn = nn.Sequential(
            nn.MaxPool2d(2, 2), nn.Conv2d(nc, 64, 3, 1, 1), nn.BatchNorm2d(64),
            nn.ReLU(True), nn.MaxPool2d(2, 2), nn.Conv2d(64, 128, 3, 1, 1),
    ...
    def forward(self, x, test, enhance=1, debug=False):
        ...
        if not test and np.random.random() > 0.5:
            return nn.functional.upsample(
                x, size=(self.targetH, self.targetW), mode='bilinear')
        if not test:
            enhance = 0
        ...
        x_small = nn.functional.upsample(
            x, size=(self.targetH, self.targetW), mode='bilinear')
        ...
        if debug:
            ...
            for i in range(x.size(0)):
                ...
                img_offsets = (offsets_grid[i]).view(
                    1, self.targetH,
                    self.targetW).data.cpu().add_(-v_min).mul_(
                        1. / (v_max - v_min))
                ...
                total_img[0:self.targetH, self.targetW + 5:2 * self.targetW +
                          5] = img_copy
