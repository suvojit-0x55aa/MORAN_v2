...
parser.add_argument(
    '--niter', type=int, default=10, help='number of epochs to train for')
parser.add_argument(
    '--lr',
    type=float,
    default=0.01,
...
parser.add_argument(
    '--MORAN', default='', help="path to model (to continue training)")
parser.add_argument(
    '--alphabet',
    type=str,
    default=
...
parser.add_argument(
    '--saveInterval', type=int, default=10000, help='Interval to be displayed')
parser.add_argument(
    '--adam',
    action='store_true',
...
parser.add_argument(
    '--sgd',
    action='store_true',
    help='Whether to use sgd (default is rmsprop)')
parser.add_argument(
    '--BidirDecoder', action='store_true', help='Whether to use BidirDecoder')
...
train_nips_dataset = dataset.lmdbDataset(
    root=opt.train_nips,
...
if opt.adam:
    optimizer = optim.Adam(
        MORAN.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
elif opt.adadelta:
...
def val(dataset, criterion, max_iter=1000):
    ...
    for i in range(max_iter):
        ...
        if opt.BidirDecoder:
            ...
            cost = criterion(
                torch.cat([preds0, preds1], 0), torch.cat([text, text_rev], 0))
    ...
    while i < len(train_loader):
        ...
        if i % opt.valInterval == 0:
            ...
            if acc_tmp > acc:
                acc = acc_tmp
                torch.save(
                    MORAN.state_dict(), '{0}/{1}_{2}.pth'.format(
                        opt.experiment, i,
                        str(acc)[:6]))
